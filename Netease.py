...
def wordCloud(wordlist):
    ...
    alice_coloring = np.array(Image.open(os.path.join(d, "李志.png")))
...
def get_hot_comments(url):
    ...
    for item in hot_comments:
            ...
            comment_info = nickname + " " + comment + u"\n"
...
def get_all_comments(url):
    ...
    txt=" ".join(comment_list)
    import jieba
    wordlist_jieba = jieba.cut(txt, cut_all=False)
    wl_space_split = " ".join(wordlist_jieba)
    return (all_comments_list,wl_space_split)
...
if __name__ == "__main__":
    ...
    # url='/Users/henry/Desktop/热河.txt'
    # comments=read_text(url)
    # wordCloud(comments)